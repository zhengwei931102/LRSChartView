...
//    [self.scrollBgView1.layer addSublayer:baseLayer];
    ...
    //渐变线
    CAGradientLayer * gradientLayerline = [CAGradientLayer layer];
    //    gradientLayerline = (id)[CAGradientLayer layer];
    gradientLayerline.colors = [NSArray arrayWithObjects:(id)[[UIColor colorWithRed:(float)(83/255.0f) green:(float)(2/255.0f) blue:(float)(254/255.0f) alpha:1.0f]CGColor],(id)[[UIColor colorWithRed:(float)(255/255.0f) green:(float)(0/255.0f) blue:(float)(0/255.0f) alpha:1.0f]CGColor], nil];
    gradientLayerline.startPoint= CGPointMake(1, 0);
    gradientLayerline.endPoint = CGPointMake(0, 0);
    //    gradientLayerline = @[@(0.25), @(0.5), @(0.75)];
    gradientLayerline.frame = CGRectMake(0, 0, self.scrollBgView1.layer.frame.size.width, self.scrollBgView1.layer.frame.size.height);
    
    [gradientLayerline setMask:shapeLayer];
    [self.scrollBgView1.layer addSublayer:gradientLayerline];
...
-(void)addDataPointWith:(UIView *)view andArr:(NSArray *)leftData{
    ...
    for (int i = 0; i<arr.count; i++) {
        ...
        float tempHeight = [arr[i] floatValue] * (5.0/6) / tempMax ;
        //zw 横坐标
...
-(void)addBottomViewsWith:(UIView *)UIView{
    ...
    for (int i = 0;i< bottomArr.count ;i++ ) {
        //zw 横坐标
//        UILabel *leftLabel = [[UILabel alloc]initWithFrame:CGRectMake(5+Xmargin/2+(i-1)*Xmargin, 6*Ymargin, Xmargin, 20)];
